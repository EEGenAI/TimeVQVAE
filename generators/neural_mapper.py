...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(
            t, 'b (h c) n -> b h c n', h=self.heads), qkv)
...
class Unet1D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        block_klass = partial(
            ResnetBlock, groups=resnet_block_groups, dropout=dropout)
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Downsample(dim_in, dim_out) if not is_last else nn.Conv1d(
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Upsample(dim_out, dim_in) if not is_last else nn.Conv1d(
        ...
        self.final_conv = nn.Sequential(nn.Conv1d(dim, self.out_dim, kernel_size=1),
                                        nn.Conv1d(
                                            self.out_dim, self.out_dim, kernel_size=3, padding=1, padding_mode='replicate'),
                                        nn.Conv1d(
                                            self.out_dim, self.out_dim, kernel_size=3, padding=1, padding_mode='replicate'),
                                        )
    ...
    def forward(self, x):
        ...
        for block1, block2, attn, upsample in self.ups:
            h_ = F.interpolate(
                h.pop(), size=x.shape[-1], mode='linear', align_corners=False)
            ...
            x = upsample(x)
        x = self.last_up(x)
        x = F.interpolate(x, size=r.shape[-1],
...
class NeuralMapper(nn.Module):
    ...
    def forward(self, x_a):
        ...
        :return:
        """
        x_a = F.upsample(x_a, size=self.input_length,
                         mode='linear', align_corners=False)
