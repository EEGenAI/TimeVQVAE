...
def load_args():
    ...
    parser.add_argument('--gpu_device_ind', nargs='+',
                        default=[0], type=int, help='Indices of GPU devices to use.')
...
def train_stage_neural_mapper(config: dict,
    ...
    train_exp = ExpNeuralMapper(dataset_name, in_channels, input_length,
                                config, n_classes, feature_extractor_type, use_custom_dataset)

    n_trainable_params = sum(p.numel()
                             for p in train_exp.parameters() if p.requires_grad)
    wandb_logger = WandbLogger(project=project_name, name=None, config={
                               **config, 'dataset_name': dataset_name, 'n_trainable_params': n_trainable_params})
    ...
    train_exp.search_optimal_tau(
        X_train=train_data_loader.dataset.X, device=eval_device, wandb_logger=wandb_logger)
...
if __name__ == '__main__':
    ...
    for dataset_name in dataset_names:
        ...
        # train
        train_stage_neural_mapper(config, dataset_name, train_data_loader, test_data_loader,
                                  args.gpu_device_ind, args.feature_extractor_type, args.use_custom_dataset)
