...
def load_args():
    ...
    parser.add_argument('--dataset_names', nargs='+',
                        help="e.g., Adiac Wafer Crop`.", default='')
    parser.add_argument('--gpu_device_idx', default=0, type=int)
    parser.add_argument('--use_neural_mapper', type=str2bool,
                        default=False, help='Use the neural mapper')
    parser.add_argument('--feature_extractor_type', type=str,
                        default='rocket', help='supervised_fcn | rocket for evaluation.')
    parser.add_argument('--use_custom_dataset', type=str2bool, default=False,
                        help='Using a custom dataset, then set it to True.')
    parser.add_argument('--sampling_batch_size', type=int,
                        default=None, help='batch size when sampling.')
...
def evaluate(config: dict,
    ...
    (_, _, xhat), xhat_R = evaluation.sample(max(
        evaluation.X_test.shape[0], min_num_gen_samples), 'unconditional', batch_size=sampling_batch_size)
    ...
    evaluation.log_visual_inspection(
        evaluation.X_train, xhat, 'X_train vs Xhat')
    ...
    if use_neural_mapper:
        ...
        evaluation.log_visual_inspection(xhat[[0]], xhat_R[[
                                         0]], 'xhat vs xhat_R', alpha=1., n_plot_samples=1)  # visaulize a single pair
        ...
        wandb.log({'MDD with NM': mdd, 'ACD with NM': acd,
                  'SD with NM': sd, 'KD with NM': kd})
    ...
    for cls_idx in range(n_classes):
        ...
        z_test_c = evaluation.compute_z_gen(
            torch.from_numpy(evaluation.X_test[cls_sample_ind]))
        ...
        axes1[cls_idx].plot(X_test_c[sample_ind, 0, :].T,
                            alpha=alpha, color='C0')
        axes1[cls_idx].set_title(f'cls_idx:{cls_idx}')
        axes1[cls_idx].set_ylim(*ylim)

        sample_ind = np.random.randint(
            0, xhat_c.shape[0], n_plot_samples_per_class)
        axes2[cls_idx].plot(xhat_c[sample_ind, 0, :].T,
                            alpha=alpha, color='C0')
        ...
        if use_neural_mapper:
            ...
            axes3[cls_idx].plot(xhat_c_R[sample_ind, 0, :].T,
                                alpha=alpha, color='C0')
    ...
    if use_neural_mapper:
        ...
        # Ensure x-axis labels are integers only
...
if __name__ == '__main__':
    ...
    if len(args.dataset_names) == 0:
        data_summary_ucr = pd.read_csv(
            get_root_dir().joinpath('datasets', 'DataSummary_UCR.csv'))
    ...
    for dataset_name in dataset_names:
        ...
        if not args.use_custom_dataset:
            ...
            train_data_loader, test_data_loader = [build_data_pipeline(
                batch_size, dataset_importer, config, kind) for kind in ['train', 'test']]
        else:
            dataset_importer = DatasetImporterCustom(**config['dataset'])
            train_data_loader, test_data_loader = [build_custom_data_pipeline(
                batch_size, dataset_importer, config, kind) for kind in ['train', 'test']]

        # train
        evaluate(config, dataset_name, train_data_loader, args.gpu_device_idx, args.use_neural_mapper,
                 args.feature_extractor_type, args.use_custom_dataset, args.sampling_batch_size)
