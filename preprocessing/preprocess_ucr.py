...
class DatasetImporterUCR(object):
    ...
    def __init__(self, dataset_name: str, data_scaling: bool, **kwargs):
        ...
        self.data_root = get_root_dir().joinpath(
            "datasets", "UCRArchive_2018_resplit", dataset_name)

        # fetch an entire dataset
        df_train = pd.read_csv(self.data_root.joinpath(
            f"{dataset_name}_TRAIN.tsv"), sep='\t', header=None)
        df_test = pd.read_csv(self.data_root.joinpath(
            f"{dataset_name}_TEST.tsv"), sep='\t', header=None)

        self.X_train, self.X_test = df_train.iloc[:, 1:].values[:, np.newaxis,
                                                                :], df_test.iloc[:, 1:].values[:, np.newaxis, :]  # (b 1 l)
        self.Y_train, self.Y_test = df_train.iloc[:, [
            0]].values[:, np.newaxis, :], df_test.iloc[:, [0]].values[:, np.newaxis, :]  # (b 1 l)
        ...
        if data_scaling:
            self.mean = np.nanmean(self.X_train, axis=(0, 2))[
                None, :, None]  # (1 c 1)
            self.std = np.nanstd(self.X_train, axis=(0, 2))[
                None, :, None]  # (1 c 1)
...
class DatasetImporterCustom(object):
    def __init__(self, data_scaling: bool = True, **kwargs):
        ...
        self.X_train = 2*np.sin(np.linspace(0, 2 * np.pi, ts_length) + np.random.rand(
            n_train_samples, n_channels, 1) * 2 * np.pi)  # (n_training_samples, n_channels, length) = (b c l)
        self.Y_train = np.random.randint(0, 2, size=(
            n_train_samples, 1))  # (n_training_samples 1)

        # Generate sine time series with random phases for test data
        self.X_test = 2*np.sin(np.linspace(0, 2 * np.pi, ts_length) + np.random.rand(
            n_test_samples, n_channels, 1) * 2 * np.pi)  # (n_test_samples, n_channels, length) = (b c l)
        self.Y_test = np.random.randint(0, 2, size=(
            n_test_samples, 1))  # (n_test_samples 1)
