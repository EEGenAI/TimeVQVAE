...
def sample(batch_size: int, maskgit, device, n_samples: int, kind: str, class_index: Union[None, int]):
    ...
    if kind == 'unconditional':
        x_new_l, x_new_h, x_new = unconditional_sample(
            maskgit, n_samples, device, batch_size=batch_size)  # (b c l); b=n_samples, c=1 (univariate)
    elif kind == 'conditional':
        x_new_l, x_new_h, x_new = conditional_sample(
            maskgit, n_samples, device, class_index, batch_size)  # (b c l); b=n_samples, c=1 (univariate)
    else:
        raise ValueError
...
class Metrics(object):
    ...
    def extract_feature_representations(self, x: np.ndarray, feature_extractor_type: str = None):
        ...
        x: (b 1 l)
        """
        feature_extractor_type = feature_extractor_type if not isinstance(
            feature_extractor_type, type(None)) else self.feature_extractor_type

        if feature_extractor_type == 'supervised_fcn':
            device = next(self.fcn.parameters()).device
            z = self.fcn(torch.from_numpy(x).float().to(
                device), return_feature_vector=True).cpu().detach().numpy()  # (b d)
    ...
    def compute_z_stat(self, x: np.ndarray) -> np.ndarray:
        ...
        z_mu, z_std = np.mean(zs, axis=0)[None, :], np.std(
            zs, axis=0)[None, :]  # (1 d), (1 d)
        return z_mu, z_std
    ...
    def inception_score(self, x_gen: np.ndarray):
        ...
        for i in range(n_iters):
            ...
            p_yx_g = self.fcn(torch.from_numpy(
                x_gen[s]).float().to(device))  # p(y|x)
