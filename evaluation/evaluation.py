...
from .stat_metrics import marginal_distribution_difference, auto_correlation_difference, skewness_difference, kurtosis_difference
...
class Evaluation(nn.Module):
    ...
    def __init__(self,
        ...
        assert feature_extractor_type in [
            'supervised_fcn', 'rocket'], 'unavailable feature extractor type.'
        ...
        dataset_importer = DatasetImporterUCR(
            dataset_name, **config['dataset']) if not use_custom_dataset else DatasetImporterCustom(**config['dataset'])
        ...
        self.xmin_pca, self.xmax_pca = np.min(
            z_transform_pca[:, 0]), np.max(z_transform_pca[:, 0])
        self.ymin_pca, self.ymax_pca = np.min(
            z_transform_pca[:, 1]), np.max(z_transform_pca[:, 1])
    ...
    def sample(self, n_samples: int, kind: str, class_index: Union[int, None] = None, unscale: bool = False, batch_size=None):
        ...
        if kind == 'unconditional':
            x_new_l, x_new_h, x_new = unconditional_sample(self.maskgit, n_samples, self.device, batch_size=batch_size if not isinstance(
                batch_size, type(None)) else self.batch_size)  # (b c l); b=n_samples, c=1 (univariate)
        elif kind == 'conditional':
            x_new_l, x_new_h, x_new = conditional_sample(self.maskgit, n_samples, self.device, class_index, batch_size=batch_size if not isinstance(
                batch_size, type(None)) else self.batch_size)  # (b c l); b=n_samples, c=1 (univariate)
        else:
            raise ValueError
    ...
    def _extract_feature_representations(self, x: np.ndarray):
        ...
        if self.feature_extractor_type == 'supervised_fcn':
            z = self.fcn(torch.from_numpy(x).float().to(
                self.device), return_feature_vector=True).cpu().detach().numpy()  # (b d)
    ...
    def compute_z_rec(self, kind: str):
        ...
        for i in range(n_iters):
            ...
            x_rec = self.stage1.forward(batch=(
                x, None), batch_idx=-1, return_x_rec=True).cpu().detach().numpy().astype(float)  # (b 1 l)
    ...
    def compute_z_svq(self, kind: str):
        ...
        for i in range(n_iters):
            ...
            _, s_a_l = self.maskgit.encode_to_z_q(
                x, self.stage1.encoder_l, self.stage1.vq_model_l, svq_temp=tau)  # (b n)
            _, s_a_h = self.maskgit.encode_to_z_q(
                x, self.stage1.encoder_h, self.stage1.vq_model_h, svq_temp=tau)  # (b m)
            x_a_l = self.maskgit.decode_token_ind_to_timeseries(
                s_a_l, 'lf')  # (b 1 l)
            x_a_h = self.maskgit.decode_token_ind_to_timeseries(
                s_a_h, 'hf')  # (b 1 l)
    ...
    def compute_z_gen(self, X_gen: torch.Tensor) -> np.ndarray:
        ...
        for i in range(n_iters):
            ...
            z_g = self._extract_feature_representations(
                X_gen[s].numpy().astype(float))
    ...
    def log_pca(self, Zs: List[np.ndarray], labels: List[str], n_plot_samples: int = 1000):
        ...
        for Z, label in zip(Zs, labels):
            ind = np.random.choice(
                range(Z.shape[0]), size=n_plot_samples, replace=True)
    ...
    def log_tsne(self, n_plot_samples: int, X_gen, z_test: np.ndarray, z_gen: np.ndarray):
        ...
        sample_ind_test = np.random.randint(
            0, self.X_test.shape[0], n_plot_samples)
        sample_ind_gen = np.random.randint(0, X_gen.shape[0], n_plot_samples)

        # TNSE: data space
        X = np.concatenate((self.X_test.squeeze()[sample_ind_test], X_gen.squeeze()[
                           sample_ind_gen]), axis=0).squeeze()
        labels = np.array(['C0'] * len(sample_ind_test) +
                          ['C1'] * len(sample_ind_gen))
        ...
        # TNSE: latent space
        Z = np.concatenate(
            (z_test[sample_ind_test], z_gen[sample_ind_gen]), axis=0).squeeze()
        labels = np.array(['C0'] * len(sample_ind_test) +
                          ['C1'] * len(sample_ind_gen))
