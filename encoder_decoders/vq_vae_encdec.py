    ...
    def __init__(self, in_channels, out_channels, frequency_indepence: bool, mid_channels=None, dropout: float = 0.):
        ...
        layers = [
            ...
            weight_norm(nn.Conv2d(mid_channels, mid_channels, kernel_size=kernel_size, stride=(
                1, 1), padding=padding, groups=mid_channels)),
            weight_norm(nn.Conv2d(mid_channels, out_channels, kernel_size=1)),
            nn.Dropout(dropout)
        ]
        self.convs = nn.Sequential(*layers)
        self.proj = nn.Identity() if in_channels == out_channels else nn.Conv2d(
...
class VQVAEDecBlock(nn.Module):
    def __init__(self,
        ...
        self.block = nn.Sequential(
            weight_norm(nn.ConvTranspose2d(in_channels, in_channels, kernel_size=kernel_size, stride=(
    ...
    def __init__(self,
        ...
        for _ in range(int(round(np.log2(downsample_rate))) - 1):
            ...
            for _ in range(n_resnet_blocks):
                enc_layers.append(
                    ResBlock(d, d, frequency_indepence, dropout=dropout))
            d *= 2
        enc_layers.append(
...
class VQVAEDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        dec_layers = [
            ResBlock(hid_dim, d, frequency_indepence, dropout=dropout)]
        for _ in range(int(round(np.log2(downsample_rate))) - 1):
            for _ in range(n_resnet_blocks):
                dec_layers.append(
                    ResBlock(d, d, frequency_indepence, dropout=dropout))
        ...
        dec_layers.append(nn.Sequential(nn.ConvTranspose2d(num_channels, num_channels, kernel_size=kernel_size, stride=(1, 2), padding=padding),
                                        nn.ConvTranspose2d(
                                            num_channels, num_channels, kernel_size=1)
                                        ))
    ...
    def forward(self, x):
        ...
        if self.kind == 'lf':
            zeros = torch.zeros(
                (out.shape[0], out.shape[1], self.n_fft//2+1, out.shape[-1])).float().to(out.device)
            zeros[:, :, [0], :] = out
            out = zeros
        elif self.kind == 'hf':
            zeros = torch.zeros(
